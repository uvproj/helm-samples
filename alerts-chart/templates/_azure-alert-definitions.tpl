...
{{- /* Setting authorization loss alert based on Transaction Response Type
AuthorizationError: The request failed due to authorization issue (e.g. Missing/Removed Role assignment). */ -}}
{{- $authorizationErrorResponseTypes := dict 
    "name" "ResponseType"
    "operator" "Include"
    "values" (list "AuthorizationError" )
-}}
{{- $authLossCriteria := dict 
    "metricName" "Transactions"
    "metricNamespace" "microsoft.storage/storageaccounts"
    "operator" "GreaterThan"
    "threshold" 0
    "timeAggregation" "Total"
    "dimensions" (list $authorizationErrorResponseTypes)
-}}
{{- $storageAccountAuthLossAlert := dict 
    "global" .global
    "name" (printf "authorization-loss-%s" .storageAccountName)
    "annotations" $annotations
    "criteria" $authLossCriteria
    "evaluationFrequency" "PT1M"
    "owner" $owner
    "scopeReference" $scopeReference
    "severity" 2 
    "windowSize" "PT5M"
    "description" (printf "Requests to Storage account '%s' not authorized properly." .storageAccountName | quote)
-}}
{{- $authLossDynamicCriteria := dict 
    "metricName" "Transactions"
    "metricNamespace" "microsoft.storage/storageaccounts"
    "operator" "GreaterThan"
    ...
    "numberOfLookbackPoints" 3
    "dimensions" (list $authorizationErrorResponseTypes)
-}}
{{- $storageAccountAuthLossDynamicAlert := dict 
    "global" .global
    "name" (printf "authorization-loss-dynamic-%s" .storageAccountName)
    "annotations" $annotations
    "criteria" $authLossDynamicCriteria
    "evaluationFrequency" "PT1M"
    "owner" $owner
    "scopeReference" $scopeReference
    "severity" 2 
...
{{- include "custom.aso2.metricDynamicAlert" $storageAccountAuthLossDynamicAlert -}}
